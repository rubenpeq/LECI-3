...
GMainLoop * loop = 0;
static GHashTable * found_devices = 0;
static guint device_count = 0;
		     ...
		     )
{
    ...
    if (device) {
        ...
        if (address) {
            const gchar * address_string = g_variant_get_string( address, NULL );

            if (!g_hash_table_contains( found_devices, address_string )) {
                g_hash_table_add( found_devices, g_strdup( address_string ) );
                device_count++;

                printf( "Found device #%u (paired: %c, bonded = %c, connected = %c, trusted = %c): %s",
                        device_count,
                        boolean_variant_char( paired ),
...
main( int argc, char * argv[] )
{
    ...
    GVariant * result ;
    GError * error = 0;
    unsigned int scan_duration = 20;

    if (argc > 2) {
        fprintf( stderr, "Usage: %s [scan-seconds]\n", argv[0] );
        return 1;
    }
    ...
    if (argc == 2) {
        ...
        if (end == argv[1] || *end != '\0' || value == 0) {
            fprintf( stderr, "Invalid scan duration: %s\n", argv[1] );
            fprintf( stderr, "Usage: %s [scan-seconds]\n", argv[0] );
            return 1;
        }

        scan_duration = (unsigned int) value;
    }

    found_devices = g_hash_table_new_full( g_str_hash, g_str_equal, g_free, NULL );
    ...
    */
    ...
    printf( "Total different devices found: %u\n", device_count );
